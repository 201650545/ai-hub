...
def get_key(channel_id):
    """渠道 key：环境变量优先，其次 config/channels.json（项目级），最后本地 channels.json。"""
    ...
    if env_name:
        ...
        if v:
            return v
    # 项目级共享配置（gitignore 不入库）
    try:
        with open(PROJECT_CONFIG_JSON, "r", encoding="utf-8") as f:
            v = json.load(f).get("keys", {}).get(channel_id, "")
            if v:
                return v
    except Exception:  # noqa: BLE001
        pass
