...
RULE_CARD_DIR = os.path.join(_THIS_DIR, "rules")
...
def run_pipeline(topic: str, lesson: str = "", style: str = "",
    ...
    """
    # 1. 确定输出目录（默认 仓库根/data/orchestrator/，数据与代码分离）
    lid = lesson.strip() or f"L{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
    if not lesson_dir:
        _DATA_DIR = os.path.join(os.path.dirname(_THIS_DIR), "..", "data", "orchestrator")
        lesson_dir = os.path.normpath(os.path.join(_DATA_DIR, lid))
